...
class_converted <- data %>%
  mutate(Month = as_factor(lbl_clean(MONTH)),
         ...
         Occupation = as_factor(lbl_clean(OCC))
         ) %>% 
  select(-c(MONTH, ASECFLAG, FOODSTMP, SEX, COVIDTELEW, COVIDUNAW, COVIDLOOK, FAMINC, OCC, LABFORCE))
...
df <- class_converted %>% 
  ...
  select(-EDUC) %>% 
  mutate(Employed = case_when(EMPSTAT <= 01 | EMPSTAT >= 30 ~ 'Others', # NIU/armedforces
